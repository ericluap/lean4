...
The main trick is to use the per-constructor `with` combinator that is like `cases` but has only
one arm, and is thus constant in size. See `WithCtor.lean` for that construction.
