...
fun P x1 x2 =>
  Vec.casesOn x1
    (if h : x2.toCtorIdx = 0 then
      Vec.nil.with (motive := fun _ _ => Sort u_1) x2 h (P → P)
    else P)
    (fun {n} a_1 a_2 => if h : x2.toCtorIdx = 1 then
      Vec.cons.with (motive := fun _ _ => Sort u_1) x2 h fun {n_1} a a_3 => (n = n_1 → a_1 = a → a_2 ≍ a_3 → P) → P
     else P)
...
fun {α} {a} P x1 x2 =>
  Vec.casesOn x1 (if h : x2.toCtorIdx = 0 then Vec.nil.with x2 h (P → P) else P) fun {n} a_1 a_2 =>
    if h : x2.toCtorIdx = 1 then Vec.cons.with x2 h fun {n_1} a a_3 => (n = n_1 → a_1 = a → a_2 ≍ a_3 → P) → P else P
-/
#guard_msgs in
